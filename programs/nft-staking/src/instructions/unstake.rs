...
pub fn handler(ctx: Context<Unstake>) -> Result<()> {
    ...
    let cuurent_timestamp = Clock::get()?.unix_timestamp;
    ...
    for attribute   in &attributes.attribute_list{
        if attribute.key == "staked"{
            ...
            staked_time = cuurent_timestamp.checked_sub(staked_timestamp).ok_or(ErrorCode::InvalidTimeStamp)?;
            //convert the staked time to days
            staked_time = staked_time.checked_div(SECOND_PER_DAY).ok_or(ErrorCode::InvalidTimeStamp)?;
            require!(staked_time >= ctx.accounts.config.freeze_period as i64, ErrorCode::FreezePeriodNotOver);
        }
        else{
